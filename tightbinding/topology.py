...
def braket(arr1:xr.DataArray, arr2:xr.DataArray, dim:str = 'component')->xr.DataArray:
...
def connect_indices_k(start:np.ndarray, stop:np.ndarray, ax:int=0)->tuple[np.ndarray,np.ndarray]:
    """
...
def berry_curvature(eigve: xr.DataArray, dims:list[str] = ["kx","ky"], compdim:str = 'component', periodic = False) -> xr.DataArray:
...
def euler_curvature(eigve: xr.DataArray, dims:list[str] = ["kx","ky"], compdim:str = 'component') -> xr.DataArray:
    """
    ...
    euler = berry.sel(band=berry.band[:-1]) * 0 # Initialize an empty euler curvature array
        ...
        for gap in euler.gap.values:
            ...
            for u in range(2):
                if len(Nodes_tuple[u][0])%2 ==1:
                    Nodes_tuple[u][0] = np.append(Nodes_tuple[u][0], Nodes_tuple[u][0][-1])
            ...
            for u in range(0,2):
                ...
                KxLinkt = xr.DataArray(KyLink[u], {dims[0]:eigve_sel.coords[dims[0]], dims[1]:eigve_sel.coords[dims[1]]})
                KyLinkt = xr.DataArray(np.flip(KxLink[u], axis = (0)), {dims[0]:eigve_sel.coords[dims[0]], dims[1]:eigve_sel.coords[dims[1]]})
                ...
                if u==0: ## Only fix the lower eigenmap with its own nodes
                    ...
                    isjump = (1 - jumpallowed * xr.ufuncs.sign(braket(jumpedge_low, jumpedge_low.shift({dims[0]: 1}))))/2
                    rectify_edge = isjump.cumsum(dim=dims[0])
                    eigve_low = xr.where(eigve_low.coords[dims[1]] == eigve_low.coords[dims[1]][0], eigve_low * (-1)**rectify_edge, eigve_low)
                if u == 1:
                    jumpedge_up = eigve_up.isel({dims[1]: 0}) # jumpedge shape (n_kx, 'components')
                    jumpallowed = KxLinkt.isel({dims[1]: 0})
                    isjump = (1 - jumpallowed * xr.ufuncs.sign(braket(jumpedge_up, jumpedge_up.shift({dims[0]: 1}))))/2
                    rectify_edge = isjump.cumsum(dim=dims[0])
                    eigve_up = xr.where(eigve_up.coords[dims[1]] == eigve_up.coords[dims[1]][0], eigve_up * (-1)**rectify_edge, eigve_up)
            
            # Now that the edge iky = 0 is fixed, we can fix the gauge
                if u == 0:
                    isjump_low = (1 - KyLinkt * xr.ufuncs.sign(braket(eigve_low, eigve_low.shift({dims[1]: 1}))))/2
                ...
                if u == 1:
                    isjump_up = (1 - KyLinkt * xr.ufuncs.sign(braket(eigve_up, eigve_up.shift({dims[1]: 1}))))/2
...
def euler_patch(eigve_low: xr.DataArray, eigve_up: xr.DataArray, dims:list[str] = ["kx","ky"], compdim:str = 'component') -> xr.DataArray:
    ...
    # We convert the link arrays to xarray for easy broadcast rules
    KxLink = xr.DataArray(KyLink, {dims[0]:eigve_low.coords[dims[0]], dims[1]:eigve_low.coords[dims[1]]})
    KyLink = xr.DataArray(np.flip(KxLink, axis = (0)), {dims[0]:eigve_low.coords[dims[0]], dims[1]:eigve_low.coords[dims[1]]})
    ...
    # Now that the edge iky = 0 is fixed, we can fix the gauge
    isjump_low = (1 - KyLink * xr.ufuncs.sign(braket(eigve_low, eigve_low.shift({dims[1]: 1}))))/2
    rectify_y_low = isjump_low.cumsum(dim=dims[1])
    
    eigve_low = eigve_low * (-1)**rectify_y_low
    isjump_up = (1 - KyLink * xr.ufuncs.sign(braket(eigve_up, eigve_up.shift({dims[1]: 1}))))/2
    rectify_y_up = isjump_up.cumsum(dim=dims[1])
    eigve_up = eigve_up * (-1)**rectify_y_up
    ...
    Ax = braket(eigve_low, eigve_up.shift({dims[0]:1}) - eigve_up.shift({dims[0]:-1})) / 2
    Ay = braket(eigve_low, eigve_up.shift({dims[1]:1}) - eigve_up.shift({dims[1]:-1})) / 2
