...
def plot_band_cuts(bands:xr.DataArray, dims = ['kx', 'ky'], contourindex:int = 0, res:int = 100):
    ...
    for i in bands.coords['band'].values:
        ...
        main_fig.add_trace(cut, col = 2, row = 1)
    ...
    bandcut_widget.update_layout(
        xaxis1=dict(
        range=[bands.coords[dims[0]].values.min(), bands.coords[dims[0]].values.max()],
        scaleanchor="y1"
        ),
        yaxis1=dict(
            range=[bands.coords[dims[1]].values.min(), bands.coords[dims[1]].values.max()],
            scaleratio=1
        ),
    ...
    def update(**kwargs):
        ...
        values_contour = np.transpose(bands.sel(control_selection, method = 'nearest').values)
        ...
        with bandcutloc.batch_update():
            ...
            for i in bands.coords['band'].values:
                ...
                datacut = (bands.sel(cut_select, method = 'nearest')).interp(cut_interp).values
    ...
    layout_box = VBox([allsliders[dim] for dim in slider_dims] +
        [HBox([allsliders[dims[1]+"1"], allsliders[dims[1]+"2"], bandcut_widget]),  # vertical slider + figure
        allsliders[dims[0]+"2"], allsliders[dims[0]+"1"]                   # horizontal slider below
    ])
...
def plot_eigenvectors(plots:list[list[xr.DataArray]], 
    ...
    n_rows = len(plots)
    n_cols = len(plots[0])
    if len(templates) != n_rows or len(eigvas) != n_rows: raise ValueError("different shapes for plots and templates")
    if len(templates[0]) != n_cols or len(eigvas[0]) != n_cols: raise ValueError("different shapes for plots and templates")
    for i in range(1, n_rows):
        ...
        if len(eigvas[i]) != n_cols: raise ValueError(f"Length of row {i} of 'eigvas' not consistent")
    

    def create_heatmap(tp:xr.DataArray, 
    ...
    for i in range(n_rows):
        for j in range(n_cols):
            ...
            cbdict = {**template["colorbar"], "len":0.7/n_rows, "x":min(1,x_cb[j]), "y":min(1,y_p[i])}
            for trace in traces:
    ...
    main_fig.update_layout(width= 400 * n_cols, height= 350 * n_rows,)
