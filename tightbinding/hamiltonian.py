...
def check_valid_coord(coord)->bool:
    ...
    isgoodtype = (type(coord) == xr.DataArray or type(coord) == np.ndarray)
    if not isgoodtype:
        return False
...
class Hopping:
    ...
    the same lattice
    """
    def __init__(self, lattice:Lattice, expr, orbname1:str, orbname2:str, displacement:tuple[int]):
    ...
    def __repr__(self):
        return (f"Coupling {self.orb1} to {self.orb2},"+
    ...
    def number_of_jumps(self, start_uc:str)->tuple[np.ndarray[int], np.ndarray[float]]:
    ...
    def end_unit_cell(self, start_uc:str) -> tuple[str,bool,bool]:
        ...
        end_uc_array = np.mod(start_uc_array+ 
                        np.array(self.disp)- 
                        self.lattice.mincoords, 
                        self.lattice.range) + self.lattice.mincoords
        ...
        orb1_exists = isinstance(self.lattice.orbitals_index.get(f"{start_uc}_{self.orb1}"), int)
        orb2_exists = isinstance(self.lattice.orbitals_index.get(f"{end_uc}_{self.orb2}"), int)
...
class Energy:
    ...
    def __init__(self, lattice:Lattice):
        ...
        self.lattice = lattice
        self.onsite_energies: dict[str] = {orb:"0" for orb in lattice.orbitals_index} # An index of each orbital's on site energy, in a symbolic expression form
        ...
        if not nunitcell:        
                ...
                if not nsite:
                    for name_site, site in uc.sites.items():
                        ...
                        if eval(condition, {"__builtins__": {}}, self.context):
                            if norbital:
                                    self.onsite_energies.update({f"{name_uc}_{name_site}_{norbital}": expr})
                            else:
                                for name_orb in site.orbitals:
                                    self.onsite_energies.update({f"{name_uc}_{nsite}_{name_orb}": expr})
                else:
                    site = uc.sites[nsite]
                    position = uc.position + site.position
                    self.context.update({"position": position, "index": index})
                    if eval(condition, {"__builtins__": {}}, self.context):
                        if norbital:
                            self.onsite_energies.update({f"{name_uc}_{nsite}_{norbital}": expr})
                        else:
            ...
            if not nsite:
                for name_site, site in uc.sites.items():
                    ...
                    if eval(condition, {"__builtins__": {}}, self.context):
                        if norbital:
                                self.onsite_energies.update({f"{nunitcell}_{name_site}_{norbital}": expr})
                        else:
                            for name_orb, orb in site.orbitals.items():
                                self.onsite_energies.update({f"{nunitcell}_{name_site}_{name_orb}": expr})
            else:
                site = uc.sites[nsite]
                position = uc.position + site.position
                self.context.update({"position": position, "index": index})
                if eval(condition, {"__builtins__": {}}, self.context):
                    if norbital:
                        self.onsite_energies.update({f"{nunitcell}_{nsite}_{norbital}": expr})
                    else:
                        for name_orb, orb in site.orbitals.items():
                            self.onsite_energies.update({f"{nunitcell}_{nsite}_{name_orb}": expr})
            ...
            for name_uc, uc in self.lattice.unitcells.items():
                ...
                if not nsite:
                    for name_site, site in uc.sites.items():
                        ...
                        if eval(condition, {"__builtins__": {}}, self.context):
                            if norbital:
                                    self.onsite_energies[f"{name_uc}_{name_site}_{norbital}"] += "+" + expr
                            else:
                                raise ValueError("At least one of nunitcell, nsite and norbital must be given")
                    ...
                    if eval(condition, {"__builtins__": {}}, self.context):
                        if norbital:
                            self.onsite_energies[f"{name_uc}_{nsite}_{norbital}"] += "+" + expr
                        else:
                            for name_orb, orb in site.orbitals.items():
                                self.onsite_energies[f"{name_uc}_{nsite}_{name_orb}"] += "+" + expr
        else:
            uc = self.lattice.unitcells[nunitcell]
            index = stringtoarray(nunitcell)
            if not nsite:
                for name_site, site in uc.sites.items():
                    ...
                    if eval(condition, {"__builtins__": {}}, self.context):
                        if norbital:
                                self.onsite_energies[f"{nunitcell}_{name_site}_{norbital}"] += "+" + expr
                        else:
                            for name_orb, orb in site.orbitals.items():
                                self.onsite_energies[f"{nunitcell}_{name_site}_{name_orb}"] += "+" + expr
            else:
                site = uc.sites[nsite]
                position = uc.position + site.position
                self.context.update({"position": position, "index": index})
                if eval(condition, {"__builtins__": {}}, self.context):
                    if norbital:
                        self.onsite_energies[f"{nunitcell}_{nsite}_{norbital}"] += "+" + expr
                    else:
                        for name_orb, orb in site.orbitals.items():
                            self.onsite_energies[f"{nunitcell}_{name_site}_{name_orb}"] += "+" + expr
...
class Hamiltonianbuilder:
    ...
    def create_array(self):
        ...
        self.size = tuple((len(coord) for coord in self.parameters.values() if check_valid_coord(coord)))
        self.Hamiltonian = xr.DataArray(
            np.zeros(self.size, dtype = self.htype),
            coords=[(dim,coord) for dim, coord in self.parameters.items() if check_valid_coord(coord)])
    ...
    def add_multidimensional_coord(self, name:str, coord:np.ndarray, dims:tuple[str]):
    ...
    def compute_on_site_energies(self):
        ...
        for key, index in self.lattice.orbitals_index.items():
            if key in self.onsite_energies:
                ...
                self.Hamiltonian.loc[{ "i": index, "j": index}] += eval(expr, {"__builtins__": {}}, self.context)
    ...
    def compute_single_link(self, coupling:Hopping,  start_uc:str):
        ...
        if exist and allowed:
            ...
            if self.periodicedges == 0:
                phase = 1
            if self.periodicedges>0:
                dotp = eval(f"distance[0] * {self.recoords[0]}", {"__builtins__": {}}, self.context)
                for i in range(1, len(self.recoords)):
                    dotp = dotp + eval(f"distance[{i}] * {self.recoords[i]}", {"__builtins__": {}}, self.context)
            ...
            if self.lattice.dimension == 2:
                ...
                dep1 = self.lattice.unitcells[start_uc].sites[nsite1].orbitals[norb1].angledep
                dep2 = self.lattice.unitcells[start_uc].sites[nsite2].orbitals[norb2].angledep
            ...
            self.Hamiltonian.loc[{ "i": indx_start, "j": indx_end }] += strength*phase*angledependance
            self.Hamiltonian.loc[{ "j": indx_start, "i": indx_end }] += np.conjugate(strength*phase*angledependance)
    ...
    def basis_exchange(self, matrix:Union[np.ndarray, list[list]]):
        ...
        V = xr.DataArray(np.array(matrix), coords={"i":np.arange(s[0]), "j":np.arange(s[1])})
        Vctr = xr.DataArray(np.conjugate(np.array(matrix).T), coords={"i":np.arange(s[0]), "j":np.arange(s[1])})
    ...
    def eigh(self):
        ...
        if self.htype == complex:
            eigve *= xr.apply_ufunc(np.exp, -1j*xr.apply_ufunc(np.angle, eigve)).sel({"component":0})
        else:
...
if __name__ == '__main__':
    ...
    onsites.set_energy(f"-eps0/2", nsite="A")
    onsites.set_energy(f"eps0/2", nsite="B")
